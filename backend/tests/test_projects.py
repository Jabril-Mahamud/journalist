...
def test_list_projects(auth_client, db, test_user):
    auth_client.post("/api/projects/", json={"name": "work"})
    auth_client.post("/api/projects/", json={"name": "personal"})
...
def test_user_cannot_see_other_users_projects(auth_client, db, second_user):
    auth_client.post("/api/projects/", json={"name": "user1_project"})
...
def test_update_project_color(auth_client, db):
    create_response = auth_client.post("/api/projects/", json={"name": "work"})
    project_id = create_response.json()["id"]
    
    response = auth_client.patch(f"/api/projects/{project_id}", json={"color": "#00ff00"})
    assert response.status_code == 200
...
def test_delete_project_removes_from_entries(auth_client, db, test_user):
    project_response = auth_client.post("/api/projects/", json={"name": "work"})
...
def test_update_nonexistent_project_returns_404(auth_client):
    response = auth_client.patch("/api/projects/999", json={"color": "#00ff00"})
